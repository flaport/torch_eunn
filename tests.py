...
def test_universality():
    ...
    U, _, _ = np.linalg.svd(random_state.randn(num_hidden,num_hidden) + 1j*random_state.randn(num_hidden,num_hidden))
    U_torch = torch.stack([
    ...
    for _ in range_:
        ...
        loss.backward()
        optimizer.step()
    result = np.abs(I_approx[...,0].detach().numpy() + 1j*I_approx[...,1].detach().numpy())**2
